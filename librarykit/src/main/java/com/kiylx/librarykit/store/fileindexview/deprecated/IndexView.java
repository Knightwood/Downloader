package com.kiylx.librarykit.store.fileindexview.deprecated;
...
import com.kiylx.librarykit.store.fileindexview.ui.FileListAdapter;
 ...
 */
public class IndexView extends LinearLayout implements MyClickListener {
    ...
    private void updateUI() {
        if (adapter == null) {
            adapter = new FileListAdapter(lists);
    ...
    public void onClick(@Nullable View v, int pos) {
        FileInfo data=adapter.getDataLists().get(pos);
        ...
        if ((new File(dataFilePath)).isFile()) {
            //如果这是文件，就不更新视图，直接返回
            if (clickAfter2 != null && action == SelectAction.FILE) {
                //如果是选择文件，在点击文件时会发出通知
                clickAfter2.after(dataFilePath);
            }
            return;
        }
     ...
     */
    public interface ClickAfter2 {
        void after(String path);
    }

    public void setClickAfter(ClickAfter2 after) {
        this.clickAfter2 = after;
    }
    ...
    public static List<FileInfo> getChildrenList(String parentPath, String[] filter) {
        ...
        for (File file : rootFile.listFiles()) {
            ...
            if (file.isDirectory()) {
                imgId = R.drawable.ic_folder_grey600_24dp;
            } else {
                ...
                imgId=R.drawable.ic_file_grey600_24dp;
            }
